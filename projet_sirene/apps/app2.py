...
data_fab = pd.read_sql('select * from fabrication limit 1000', conn)
...
def app():
    ...
    with col1:
            st.markdown("<h2 style='text-align: center; color: black;'>Densité des secteurs d'activité secondaire </h2>", unsafe_allow_html=True)
    with col2:
            st.image("https://cdn-icons-png.flaticon.com/512/129/129522.png", width=160)
     
    st.write(' ')
    st.write(' ')
    st.write(' ')    
     
    option_fab = st.selectbox('Selection du secteur d''activité', data_fab['intitule1'].unique())
    st.write('Vous avez sélectionné : ', option_fab)    
    
    
        
    m = folium.Map([46.227638,2.213749], zoom_start=6, tiles = "Stamen Watercolor")
                       
    HeatMap(data_fab[['latitude','longitude']].loc[data_fab.intitule1.str.contains(option_fab)]).add_to(m)
    folium.Marker(
